...
For Nuxt projects, see the dedicated [Nuxt Integration](/en/eslint-config/nuxt) guide — it pairs this config with `@nuxt/eslint` so the Nuxt module provides auto-import awareness on top.
...
This is the preferred extension point — reach for the [Config Composer](/en/eslint-config/advanced#config-composer) only when `overrides` isn't granular enough.
